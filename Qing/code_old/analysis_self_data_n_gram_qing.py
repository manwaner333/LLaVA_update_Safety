...
def extract_info_from_answers(file_path, use_tfidf_weight=False, use_attention_weight=False):
    ...
    with open('result/self_data/self_data_val.json', 'r') as f:
...
def analysis_sentence_level_self_check_info(total_ngram_scores_max, total_ngram_scores_avg, total_false_labels, ngram_flag_max, ngram_flag_avg, save_path, name):
    # True
    # uncertainty
    Pb_ngram_max, Rb_ngram_max = get_PR_with_human_labels(total_ngram_scores_max, total_false_labels, pos_label=1,
                                                oneminus_pred=ngram_flag_max)
    Pb_ngram_avg, Rb_ngram_avg = get_PR_with_human_labels(total_ngram_scores_avg, total_false_labels, pos_label=1,
                                                oneminus_pred=ngram_flag_avg)


    print("-----------------------")
    print("Baseline1: Ngram-Max")
    print_AUC(Pb_ngram_max, Rb_ngram_max)
    print("-----------------------")
    print("Baseline2: Ngram-Acg")
    print_AUC(Pb_ngram_avg, Rb_ngram_avg)
    print("-----------------------")

    random_baseline = np.mean(total_false_labels)
...
if __name__ == "__main__":
    ...
    # self_check相关数据分析
    self_check_file_path = f'result/self_data/{model_version}_answer_self_check_pope_adversarial_new_prompt_responses_denoted_n_gram_5.bin'
    ...
    total_ngram_scores_max = []
    total_ngram_scores_avg = []


    for idx, response in tqdm(self_check_responses.items()):

        sent_scores_ngrams_max = response['sent_scores_ngrams']['sent_level']['max_neg_logprob']
        sent_scores_ngrams_avg = response['sent_scores_ngrams']['sent_level']['avg_neg_logprob']

        if any(math.isinf(x) for x in sent_scores_ngrams_max) or any(math.isinf(x) for x in sent_scores_ngrams_avg):
            continue
        ...
        total_ngram_scores_max.extend(sent_scores_ngrams_max)
        total_ngram_scores_avg.extend(sent_scores_ngrams_avg)
    ...
    # False
    print("False")
    ngram_flag_max = False
    ngram_flag_avg = False
    save_path = 'result/m_hal'
    name = f'{model_version}_m_hal_self_check_false'
    analysis_sentence_level_self_check_info(total_ngram_scores_max, total_ngram_scores_avg, total_false_labels, ngram_flag_max, ngram_flag_avg, save_path, name)
