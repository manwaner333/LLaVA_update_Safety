...
def extract_info_from_answers(file_path, use_tfidf_weight=False, use_attention_weight=False):
    ...
    with open('result/coco2014_val/adversarial_val.json', 'r') as f:
...
if __name__ == "__main__":
    ...
    split = 'random'    # 'adversarial'
    print(model_version)
    path = f"result/coco2014_val/{model_version}_answer_coco_pope_{split}_extract_info_without_role.bin"
    ...
    df.to_csv(f"result/coco2014_val/{model_version}_coco2014_{split}_val_df.csv")

    # 读取数据
    df = pd.read_csv(f"result/coco2014_val/{model_version}_coco2014_{split}_val_df.csv")
