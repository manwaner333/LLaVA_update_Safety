...
def extract_activations_from_pos_cases(layer, activations_file="aa.json"):
    activations = []
    labels = []
    ...
    with open(activations_file, "r") as f:
        for line in f:
            ...
            harmful_subcategory = item['harmful_subcategory']
            prompt = item["prompt"]
            # activations_head = item["block_self_attn_heads_activations"][str(layer)][0]
            # activations_layer = list(itertools.chain(*activations_head))
...
def extract_activations_from_neg_cases(layer, activations_file="aa.json"):
    activations = []
    labels = []
...
def save_activation_projection_tsne_detail_for_pos_neg_cases(
    activations1,
    labels1,
    activations2,
    labels2,
    ...
    # pos
    pos_index = [item for item in range(len(labels1)) if labels1[item] == 1]
    pos_activations = activations1_projected[pos_index]

    # neg
    neg_index = [item for item in range(len(labels2)) if labels2[item] == 0]
    neg_activations = activations2_projected[neg_index]



    # pos
    plt.scatter(pos_activations[:, 0], pos_activations[:, 1], color="blue", marker="o", alpha=0.4, label="positive")

    # neg
    plt.scatter(neg_activations[:, 0], neg_activations[:, 1], color="red", marker="o", alpha=0.4, label="negative")
...
if __name__ == "__main__":
    # analysis activations, experiment for why lvlm is unsafe, 这里面仅仅对instructions进行降维度和聚类
    ...
    # 这里面是对生成的正面cases 和 负面 cases， 每个case 里面不仅仅包含instruction , 也包含 response, 进行降维度和聚类
    ...
    for layer in layers:
        fname = f"clustering/{save_path}/activations_layer_{layer}.png"
        title = f"t-SNE projected activations layer {layer}"
        pos_activations, pos_labels = extract_activations_from_pos_cases(layer, activations_file=pos_cases_path)
        neg_activations, neg_labels = extract_activations_from_neg_cases(layer, activations_file=neg_cases_path)
        save_activation_projection_tsne_detail_for_pos_neg_cases(pos_activations, pos_labels, neg_activations, neg_labels, fname, title)
