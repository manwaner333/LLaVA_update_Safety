...
def extract_block_layers_activations(layers, activations_file="aa.json"):
    res = {}
    for i in range(0, 32):
        res[f"layer_{i}"] = []
    for layer in layers:
        ...
        with open(activations_file, "r") as f:
            for line in f:
                ...
                match = re.match(r'layer_(\d+)', id)
                if match:
                    layer_num = int(match.group(1))
                    if layer_num == layer:
                        res[f"layer_{layer}"] = activations
    return res
...
def generate_layer_data(file1=None, file2=None):
    res = {}
    for i in range(0, 32):
        for j in range(0, 32):
            res[f"layer_{i}"] = {'activations': [], 'labels': []}

    if file1 is not None:
        with open(file1, "r") as file_1:  # train_file
            for line in file_1:
                item = json.loads(line)
                block_activations = item['activations']
                for layer_num in range(0, 32):
                    layer_activations = block_activations[str(layer_num)][0]
                    res[f"layer_{layer_num}"]['activations'].append(layer_activations)
                    res[f"layer_{layer_num}"]['labels'].append(1.0)
        ...
        with open(file2, "r") as file_2:  # train_file
            for line in file_2:
                ...
                for layer_num in range(0, 32):
                    layer_activations = block_activations[str(layer_num)][0]
                    res[f"layer_{layer_num}"]['activations'].append(layer_activations)
                    res[f"layer_{layer_num}"]['labels'].append(0.0)
    return res
...
if __name__ == "__main__":
    ...
    #
    # out_file = "result/vlguard/llava_v1.5_7b_probe_head_on_pos_nes_cases.json"
    ...
    answers_vlguard_pos_neg_file = "result/vlguard/llava_v1.5_7b_probe_head_on_pos_nes_cases.json"
    res_heads_pos_neg = extract_block_heads_activations(layers, activations_file=answers_vlguard_pos_neg_file)


    for layer in layers:
        key = f"layer_{layer}"
        res_pos_neg_sub = res_heads_pos_neg[key][0]
    ...
    # end_layer = 31
    # layers = list(range(start_layer, end_layer + 1))
    # res = generate_layer_data(pos_cases_path, neg_cases_path)
    #
    # out_file = "result/vlguard/llava_v1.5_7b_probe_layer_on_pos_nes_cases.json"
    # out_file_path = os.path.expanduser(out_file)
    # if os.path.exists(out_file_path):
    #     os.remove(out_file_path)
    #
    # with open(out_file_path, 'w') as out_file:
    #     for layer_num in range(0, 32):  # 0, 32
    #         layer_head_data = res[f"layer_{layer_num}"]
    #         activations = layer_head_data['activations']
    #         labels = layer_head_data['labels']
    #         train_data, test_data, train_labels, test_labels = train_test_split(activations, labels, test_size=0.2,
    #                                                                             random_state=42)
    ...
    #         head_vector = proj_val_std * direction
    #
    #         json.dump({"id": f"layer_{layer_num}", "best_accuracy": acc,
    #                    'activations': head_vector.tolist()}, out_file)
    #         out_file.write('\n')

    # generate vectors
    # save_path = "llava_v1.5_7b_vlguard_train_layer_level_from_probe_weight_direction"
    # if not os.path.exists(f"vectors/{save_path}"):
    #     os.mkdir(f"vectors/{save_path}")
    #
    # if not os.path.exists(f"clustering/{save_path}"):
    ...
    #
    #
    # answers_vlguard_pos_neg_file = "result/vlguard/llava_v1.5_7b_probe_layer_on_pos_nes_cases.json"
    # res_heads_pos_neg = extract_block_layers_activations(layers, activations_file=answers_vlguard_pos_neg_file)
    #
    #
    # for layer in layers:
    #     key = f"layer_{layer}"
